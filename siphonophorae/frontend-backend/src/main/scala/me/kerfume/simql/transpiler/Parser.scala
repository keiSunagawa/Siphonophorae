...
object Parser extends JavaTokenParsers {
  ...
  val number: Parser[NumberWrapper] = decimalNumber ^^ { s =>
    NumberWrapper(BigDecimal(s))
  }
  val nullLit: Parser[NullLit.type] = "null" ^^ { _ =>
    NullLit
  }
  ...
  val expr: Parser[Expr] = binaryCond ~ rep(logicalOp ~ binaryCond) ^^ {
    case b ~ body =>
  ...
  val limitOffset = "@" ~ number ~ opt("-" ~ number) ^^ {
    case _ ~ limit ~ offsetSyntax =>
      val offset = offsetSyntax.map { case _ ~ ofs => ofs }
      LimitOffset(limit, offset)
  }
  val order = orderType ~ symbolWithAccessor ~ rep(symbolWithAccessor) ^^ {
    case tpe ~ col1 ~ coln =>
      Order(tpe, col1, coln)
  }

  val simql = from ~ opt(select) ~ opt(where) ~ opt(limitOffset) ~ opt(order) ^^ {
    case f ~ s ~ w ~ l ~ o =>
      SimqlRoot(f, s, w, l, o)
  }
}
